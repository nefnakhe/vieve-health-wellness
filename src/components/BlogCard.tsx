...
export default function BlogCard({ post, featured = false }: Props) {
    ...
    return (
      ...
      >
        <div className="lg:flex">
          <div className="lg:w-1/2 relative h-64 lg:h-auto bg-gradient-to-br from-teal-700 to-teal-500 flex items-center justify-center">
            {post.image && (
          ...
          <div className="lg:w-1/2 p-8 flex flex-col justify-center">
            ...
            <h3 className="text-2xl font-bold text-gray-900 mb-3 group-hover:text-teal-400 transition-colors leading-tight">
              {post.title}
            </h3>
            <p className="text-gray-600 text-sm leading-relaxed mb-5">{post.excerpt}</p>
            <div className="flex items-center justify-between">
              <span className="text-xs text-gray-400">{post.date}</span>
              <span className="text-teal-700 font-semibold text-sm group-hover:text-teal-500 transition-colors">
                Read Article →
              </span>
            </div>
          </div>
  ...
  return (
    ...
    >
      <div className="relative h-48 bg-gradient-to-br from-teal-700 to-teal-500 overflow-hidden">
        {post.image && (
          <Image
            src={post.image}
            alt={post.title}
      ...
      <div className="p-6">
        ...
        <h3 className="font-bold text-gray-900 mb-2 group-hover:text-teal-400 transition-colors leading-snug text-lg">
          {post.title}
        </h3>
